...
import {
  listPlatformWebsiteFaqs,
  createPlatformWebsiteFaq,
  updatePlatformWebsiteFaq,
  deletePlatformWebsiteFaq,
} from "@/src/services/api/platformWebsiteFaqs.api";
...
type CategoryGroup = {
  ...
  items: PlatformWebsiteFaqRow[];
};

function faqCategoryKey(row: PlatformWebsiteFaqRow): string {
  return row.category.trim() || "Uncategorized";
}

function groupByCategory(faqs: PlatformWebsiteFaqRow[]): CategoryGroup[] {
  const map = new Map<string, PlatformWebsiteFaqRow[]>();
...
function FaqQuestionRow({
  ...
  rowDragProps: ReturnType<
    typeof useCatalogDragReorder
  >["rowDragProps"];
  onToggleActive: (row: PlatformWebsiteFaqRow, next: boolean) => void;
  onEdit: (row: PlatformWebsiteFaqRow) => void;
  onDelete: (row: PlatformWebsiteFaqRow) => void;
}) {
  return (
    <div
...
function FaqCategorySection({
  ...
  onEdit: (row: PlatformWebsiteFaqRow) => void;
  onDelete: (row: PlatformWebsiteFaqRow) => void;
  onQuestionsReordered: (categoryKey: string, rows: PlatformWebsiteFaqRow[]) => void;
}) {
  const reorderQuestions = useCallback(
    async (reordered: PlatformWebsiteFaqRow[]) => {
      ...
      try {
        const updated = await Promise.all(
          withNewOrder.map((row) =>
            updatePlatformWebsiteFaq(row.id, { sortOrder: row.sortOrder }),
          ),
        );
...
export default function FaqsPage() {
  ...
  const [faqs, setFaqs] = useState<PlatformWebsiteFaqRow[]>([]);

  const [form, setForm] = useState<FormState>(emptyForm);
  const [creating, setCreating] = useState(false);

  const [editRow, setEditRow] = useState<PlatformWebsiteFaqRow | null>(null);
  ...
  const handleQuestionsReordered = useCallback(
    (categoryKey: string, rows: PlatformWebsiteFaqRow[]) => {
      setFaqs((prev) => {
        ...
        await Promise.all(
          reordered.flatMap((group, index) =>
            group.items.map((item) =>
              updatePlatformWebsiteFaq(item.id, { categorySortOrder: index }),
            ),
          ),
        );
      } catch (e) {
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      const row = await createPlatformWebsiteFaq({
  ...
  const openEdit = (row: PlatformWebsiteFaqRow) => {
    setEditRow(row);
    setEditForm({
  ...
  const handleSaveEdit = async () => {
    ...
    setSavingEdit(true);
    try {
      const updated = await updatePlatformWebsiteFaq(editRow.id, {
  ...
  const toggleActive = async (row: PlatformWebsiteFaqRow, next: boolean) => {
    try {
      const updated = await updatePlatformWebsiteFaq(row.id, { isActive: next });
      setFaqs((p) => p.map((r) => (r.id === updated.id ? updated : r)));
    } catch (e) {
      setError(e instanceof Error ? e.message : "Update failed");
    }
  ...
  const openConfirmDelete = (row: PlatformWebsiteFaqRow) => {
    confirmActionRef.current = async () => {
      await deletePlatformWebsiteFaq(row.id);
    ...
    <>
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
        ...
        <p className="mb-5 text-theme-sm text-gray-500 dark:text-gray-400">
          Manage categories, questions and answers for the marketing website FAQs page
          (/resources/faqs). Drag categories and questions to change their order.
        </p>
